...
((Drupal, once) => {
  ...
  function transitionToDesktopNavigation(navWrapper, navItem) {
    ...
    // may not be enabled.
   /* if (document.readyState === 'complete' && navWrapper.clientHeight > navItem.clientHeight) {
      document.body.classList.add('is-always-mobile-nav');
      console.log('transitiontodesktop go back to mobile', navWrapper, navItem);
    }*/
  }
  ...
  function checkIfDesktopNavigationWraps(entries) {
    ...
    // console.log('checkifDesktopWraps fires', entries, navItem);
    transitionToDesktopNavigation(entries[0].target, navItem);
    if (
      Drupal.olivero.isDesktopNav() // &&
//      Math.floor(entries[0].contentRect.height) > navItem.clientHeight
    ) {
      const navMediaQuery = window.matchMedia(
        `(max-width: ${window.innerWidth + 5}px)`, // 5px adds a small buffer before switching back.
      );
      // console.log('wrapcheck add listen', entries[0].contentRect.height, navItem.clientHeight, navMediaQuery)
       //document.body.classList.add('is-always-mobile-nav');
      ...
      navMediaQuery.addEventListener(
        'change',
        () => {
          // console.log('transition to desktop');
          transitionToDesktopNavigation(entries[0].target, navItem);
        },
        //{ once: true },
      );
